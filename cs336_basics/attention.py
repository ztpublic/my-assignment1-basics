...
class MultiHeadAttention(torch.nn.Module):
    def __init__(
        self, d_model: int, num_heads: int, device: torch.device | None = None
    ):
        super().__init__()
...
class RopeMultiHeadAttention(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, token_positions: Int[Tensor, " ... sequence_length"] | None = None) -> torch.Tensor:
        ...
        q_rope = self.rope(q_reshape, token_positions)
        k_rope = self.rope(k_reshape, token_positions)

        v_out = scaled_dot_product_attention(q_rope, k_rope, v_reshape, mask)
